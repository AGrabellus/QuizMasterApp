...
public class MemoryErstellenView extends VBox {
    ...
    public MemoryErstellenView(Stage primaryStage, File baseFolder) {
        ...
        Text title = new Text("📝 Memory-Quiz Erstellen / Laden");
    ...
    private void handleLoadCreate(String quizName) {
        ...
        // Hauptordner für diesen Quiz-Typ anlegen, z.B. C:/QuizMasterApp/Memory
        File MemoryDir = new File(baseFolder, "Memory");
        if (!MemoryDir.exists()) {
            MemoryDir.mkdirs();
        }

        // Unterordner für Medien (Bilder etc.) anlegen, z.B. C:/QuizMasterApp/Memory/Tiere
        currentQuizMediaFolder = new File(MemoryDir, safeName);
        if (!currentQuizMediaFolder.exists()) {
            currentQuizMediaFolder.mkdirs();
        }

        // Textdatei im Memory-Ordner anlegen, z.B. C:/QuizMasterApp/Memory/Tiere.txt
        currentQuizFile = new File(MemoryDir, safeName + ".txt");
